...
export function ChatHeader(props: { title: string; connection?: 'success' | 'warning' | 'neutral'; connectionLabel?: string }) {
  return (
    <header class="flex h-13 items-center gap-2 border-b border-border-subtle bg-surface-overlay px-4">
      ...
      <Status live label={props.connectionLabel ?? 'Online'} tone={props.connection ?? 'success'} />
    </header>
  );
}
