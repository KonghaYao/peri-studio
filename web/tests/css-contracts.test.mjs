...
test('MessageList delegates entry semantics through stable entry-id slots to one tested conversation component', () => {
  ...
  assert.match(list, /<ConversationMessage[\s\S]*?entry=\{entry\}[\s\S]*?chatId=\{\(\) => selectedCid\(\) \?\? currentChatId\(\)\}[\s\S]*?activityBoundary=\{\(\) => activityBoundaryAt\(chatEntries\(\), globalIndex\(\)\)\}[\s\S]*?activityContinuation=\{\(\) => activityContinuationAt\(chatEntries\(\), globalIndex\(\)\)\}[\s\S]*?terminalNoticeOwner=\{\(\) => isTurnTerminalNoticeOwner\(chatEntries\(\), globalIndex\(\)\)\}[\s\S]*?hiddenToolBlockIds=\{\(\) => hiddenToolBlocks\(\)\.get\(entry\(\)\.id\) \?\? new Set<string>\(\)\}[\s\S]*?\/>/);
