...
describe('composerInputState', () => {
  it('keeps the field typable and only locks send before any chat exists', () => {
  ...
  it('waits for both runtime documents before sending, not before typing', () => {
  ...
  it('marks a terminal conversation as send-locked history', () => {
  ...
  it('keeps the field typable during current-session confirmation and only locks send', () => {
  ...
  it('is ready with the default placeholder when everything is available', () => {
    const state = composerInputState(base());
    expect(state.composerReady).toBe(true);
    expect(state.sendLocked).toBe(false);
    expect(state.placeholder).toBe('Message the agent, or type / for commands');
