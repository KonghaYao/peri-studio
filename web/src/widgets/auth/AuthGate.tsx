...
export function AuthGate(props: { children: JSX.Element }) {
  ...
  return (
    <Show when={auth.state() === 'signed-in'} fallback={
      <main class="grid min-h-dvh place-items-center bg-sidebar-bg p-safe">
        <Card class="m-24 w-(--container-auth-card) rounded-16 border-border-subtle p-34 shadow-auth" aria-labelledby="auth-title">
