...
export function TokenUsageMeter(props: {
  usage: TokenUsage;
  /** 上下文上限，用于余量着色；缺省由 agent contextWindow 推导。 */
  limit?: number;
  ...
  const limit = () => props.limit ?? props.contextWindow ?? 200_000;
  const used = () => input() + output() + cached();
  const ratio = () => limit() > 0 ? Math.min(1, used() / limit()) : 0;
  const state = createMemo(() => usageState(ratio()));
  const remaining = () => Math.max(0, limit() - used());
  const breakdown = () => tokenUsageLabel(props.usage);
  ...
  const values = () => ({ input: input(), output: output(), cached: cached() });
  ...
  return (
    <KPopover open={open()} onOpenChange={(next) => { if (!next) setPinned(false); }}>
      ...
      <KPopover.Portal>
        <KPopover.Content
          class="z-(--z-overlay) w-[200px] rounded-md border border-border-subtle bg-surface-overlay p-10 shadow-overlay outline-none"
          onPointerEnter={() => setHovering(true)}
          onPointerLeave={() => setHovering(false)}
        >
