...
import { notification } from '@peri/ui';
...
describe('ErrorCenter', () => {
  it('offers same-command confirmation only for registered uncertain actions', async () => {
    setPersistentErrors([
    ...
    const recovery = notices.find((node) => node.textContent?.includes('Result not confirmed'));
    const forbidden = notices.find((node) => node.textContent?.includes('No permission'));
    expect(recovery).toBeTruthy();
    expect(forbidden).toBeTruthy();
    expect(recovery).toHaveTextContent('Re-confirm with the original request');
    expect(forbidden).not.toHaveTextContent('Re-confirm with the original request');
  });

  it('surfaces one payload-free transport problem as a global notification', async () => {
