...
describe('ErrorCenter', () => {
  it('offers same-command confirmation only for registered uncertain actions', () => {
    ...
    expect(screen.getByText('Result not confirmed').closest('[data-testid="error-card"]')).toHaveAttribute('role', 'alert');
    expect(screen.getByRole('button', { name: 'Re-confirm with the original request' })).toBeInTheDocument();
    expect(screen.getByText('Result not confirmed').closest('[data-testid="error-card"]')).not.toHaveTextContent('Close');
    expect(screen.getByText('No permission').closest('[data-testid="error-card"]')).not.toHaveTextContent('Re-confirm with the original request');
  });
