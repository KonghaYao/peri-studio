...
export function ProjectSidebarPinned(props: ProjectSidebarPinnedProps) {
  return (
    <Show when={props.model.pinnedSessions().length > 0}>
      <SectionHeader title="Pinned" icon={<Pin size={14} strokeWidth={1.7} />} />
      <div data-testid="pinned-list" class="flex flex-col gap-2 pb-1">
        <For each={props.model.pinnedSessions().map((session) => session.id)}>
          {(sessionId) => {
            const session = () => props.model.pinnedSessions().find((item) => item.id === sessionId)!;
            return (
              <ProjectSidebarRow
                model={props.model}
                session={session}
