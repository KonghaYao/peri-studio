...
export function Toast<T extends ValidComponent = 'li'>(props: PolymorphicProps<T, ToastProps<T>>) {
  ...
  return <ToastPrimitive.Root class={cn('box-border w-[min(360px,calc(100vw-32px))] translate-y-0 rounded-12 border border-border-subtle bg-surface px-12 py-9 text-14 leading-14 text-text-primary opacity-100 shadow-popover transition-[opacity,transform] duration-150 ease-in motion-reduce:transition-none pointer-events-auto', local.class)} {...rest} />;
}
