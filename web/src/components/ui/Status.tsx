...
export function Status(props: Props) {
  ...
  return <div {...div} role="status" aria-live={local.live ? 'polite' : undefined} class={`flex min-w-0 items-center gap-6 text-12 text-text-muted ${local.class ?? ''}`}><span class={`h-7 w-7 shrink-0 rounded-full ${DOT_TONE_CLASS[local.tone ?? 'idle']}`} aria-hidden="true" /><span class="overflow-hidden text-ellipsis whitespace-nowrap">{local.children}</span></div>;
}
