...
export function SelectField(props: Props) {
  ...
  return <div class="mb-9 flex flex-col gap-6">
    ...
    <select {...select} id={id()} aria-invalid={local.error ? 'true' : undefined} aria-describedby={describedBy()} class={`box-border h-38 w-full appearance-auto rounded-9 border border-border-strong bg-surface px-11 text-text-primary outline-none focus:border-text-primary focus:shadow-[0_0_0_1px_var(--surface),0_0_0_3px_var(--focus-ring)] focus-visible:outline-0 ${local.class ?? ''}`}>{local.children}</select>
