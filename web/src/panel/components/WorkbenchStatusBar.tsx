...
export function WorkbenchStatusBar(props: { onOpenResources: () => void }) {
  ...
  return <footer class="workbench-status-bar col-[1/-1] row-start-2 flex h-22 min-w-0 items-center gap-12 bg-accent px-8 text-11 text-white" aria-label="Workbench status">
    ...
    <button type="button" class="ml-auto flex h-full min-w-0 items-center gap-5 border-0 bg-transparent px-6 text-inherit hover:bg-white/10 focus-visible:outline-white" aria-label="Open workspace resources" onClick={props.onOpenResources}>
      <FilesIcon />
