...
export function McpPanelContent(props: { embedded?: boolean } = {}) {
  ...
  return <section class={props.embedded ? 'ui-scrollbar min-h-0 flex-1 overflow-auto bg-surface' : 'w-[min(620px,calc(100vw-40px))] max-h-[min(76dvh,680px)] overflow-auto p-22 max-[640px]:w-[calc(100vw-24px)] max-[640px]:p-17'} aria-labelledby="mcp-panel-title">
      <div class={props.embedded ? 'resource-section-title flex h-28 items-center border-b border-divider px-8 text-10 font-650 uppercase tracking-6 text-text-secondary pointer-coarse:h-44' : 'flex items-start justify-between gap-20 max-[640px]:gap-10'}>
        <div><h2 id="mcp-panel-title" class={props.embedded ? 'm-0 text-10 font-650 uppercase tracking-6 text-text-secondary' : 'm-0 text-[19px] text-text-primary tracking-[-.02em]'}>{props.embedded ? 'Connections' : 'MCP connections'}</h2><Show when={!props.embedded}><p class="mt-5 mb-0 text-text-secondary text-13 leading-[1.5]">View the MCP services of the current Peri runtime and complete authorization as needed.</p></Show></div>
        <Show when={props.embedded} fallback={<Button size="compact" busy={mcpLoading()} class="min-w-58 shrink-0 whitespace-nowrap max-[640px]:min-h-44" onClick={refreshMcpServers}>Refresh</Button>}>
          <IconButton label="Refresh MCP connections" busy={mcpLoading()} onClick={refreshMcpServers} class="ml-auto size-24 min-h-24 border-0 bg-transparent text-text-muted pointer-coarse:size-44 pointer-coarse:min-h-44"><RefreshCw size={14} strokeWidth={1.7} /></IconButton>
        </Show>
      </div>
            ...
            return <article class={props.embedded ? 'border-b border-divider bg-surface px-8 py-7' : 'rounded-12 border border-divider bg-surface p-15'}>
              ...
              <div class={`${props.embedded ? 'mt-6 gap-4' : 'mt-13 gap-7'} flex flex-wrap`}>
                <Show when={server.oauthStatus === 'needs_authorization' && !server.activeFlowId}>
                  <Button variant="primary" size={props.embedded ? 'compact' : undefined} class={props.embedded ? 'min-h-28! text-10!' : 'max-[640px]:min-h-44 max-[640px]:flex-1'} disabled={readOnly()} onClick={() => startMcpOAuth(server.name)}>{props.embedded ? 'Authorize' : 'Start authorization'}</Button>
                </Show>
                <Show when={server.activeFlowId && flow()?.status === 'authorization_needed'}>
                  <Button variant="primary" size={props.embedded ? 'compact' : undefined} class={props.embedded ? 'min-h-28! text-10!' : 'max-[640px]:min-h-44 max-[640px]:flex-1'} disabled={readOnly()} onClick={() => requestMcpAuthorization(server.activeFlowId!)}>{props.embedded ? 'Open link' : 'Get authorization link'}</Button>
                  <Button size={props.embedded ? 'compact' : undefined} class={props.embedded ? 'min-h-28! text-10!' : 'max-[640px]:min-h-44 max-[640px]:flex-1'} disabled={readOnly()} onClick={() => cancelMcpOAuth(server.activeFlowId!)}>{props.embedded ? 'Cancel' : 'Cancel authorization'}</Button>
                </Show>
                <Show when={exactAuthorization()}>{(grant) => <a class={`inline-flex items-center justify-center rounded-8 border border-transparent bg-btn-primary font-500 text-surface no-underline cursor-pointer [transition:background_120ms_ease,border-color_120ms_ease,color_120ms_ease,transform_120ms_ease,opacity_120ms_ease] hover:bg-btn-primary-hover active:translate-y-1 pointer-coarse:min-h-44 ${props.embedded ? 'min-h-28 px-8 text-10' : 'min-h-36 gap-8 px-12 text-14 max-[640px]:min-h-44 max-[640px]:flex-1'}`} href={grant().authorizationUrl} target="_blank" rel="noopener noreferrer">Open authorization page</a>}</Show>
              </div>
