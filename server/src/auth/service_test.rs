...
#[test]
fn failure_audit_carries_total_snapshot() {
...
fn h6_role_mismatch() {
    ...
    let result = block_on(svc.authenticate_client(
        &Auth {
    ...
    let ok = block_on(svc.authenticate_client(
        &Auth {
    ...
    let ro = svc
        ...
        .unwrap();
    let ok = block_on(svc.authenticate_client(&Auth { token: ro.token }, peer())).unwrap();
