...
async fn project_archive_rejects_a_live_runtime() {
    ...
    env.chats.set_active_turn("chat-live", "turn-live").await;
...
#[tokio::test]
async fn session_archive_succeeds_with_idle_runtime() {
    let env = env().await;
    env.metadata
        .create_project("p1", "Demo", env._tmp.path().to_str().unwrap(), "local")
        .await
...
#[tokio::test]
async fn session_archive_rejects_an_active_turn() {
    let env = env().await;
    env.metadata
        .create_project("p1", "Demo", env._tmp.path().to_str().unwrap(), "local")
        .await
    ...
    seed_catalog_session(&env.projects, "p1", "acp-1", "Active").await;
    bound_session(&env, S1, "acp-1").await;
    env.chats.set_active_turn(S1, "turn-live").await;
    assert!(env.chats.has_live_acp_session("acp-1").await);
...
#[tokio::test]
async fn session_archive_succeeds_after_pending_close() {
    let env = env().await;
    env.metadata
        .create_project("p1", "Demo", env._tmp.path().to_str().unwrap(), "local")
        .await
    ...
    seed_catalog_session(&env.projects, "p1", "acp-1", "Active").await;
    bound_session(&env, S1, "acp-1").await;
    env.chats.request_close_offline(S1).await.unwrap();
    assert!(!env.chats.has_live_acp_session("acp-1").await);
