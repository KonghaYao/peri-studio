...
fn question_requested_upserts_pending_questions() {
    ...
    assert!(
        Aggregator
            .apply(&mut pair, &ev("s1", 1, question_requested("q1")))
            .applied
    );
    assert_eq!(
        pending_question_status(&pair, "q1").as_deref(),
        Some("pending")
    );

    assert!(
        Aggregator
            .apply(&mut pair, &ev("s1", 2, question_requested("q1")))
            .applied
    );
    assert_eq!(
        pending_question_status(&pair, "q1").as_deref(),
...
fn question_resolved_and_expired_persist_via_cas() {
    ...
    assert!(
        Aggregator
            .apply(&mut pair, &ev("s1", 1, question_requested("q2")))
            .applied
    );
    ...
    assert_eq!(
        pending_question_status(&pair, "q2").as_deref(),
        Some("resolved")
    );
    ...
    assert!(
        Aggregator
            .apply(&mut pair_expire, &ev("s1", 1, question_requested("q3")))
            .applied
    );
    assert!(
        Aggregator
    ...
    assert_eq!(
        pending_question_status(&pair_expire, "q3").as_deref(),
        Some("expired")
    );
}
...
fn replay_question_first_projects_pending_without_synthesizing_turn() {
    ...
    assert!(
        Aggregator
            .apply(&mut pair, &ev("s1", 1, question_requested("q-replay")))
            .applied
    );
    assert_eq!(
