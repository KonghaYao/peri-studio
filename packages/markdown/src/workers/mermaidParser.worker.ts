...
function findHeaderIndex(lines: string[]) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (headerRe.test(line)) return index;
  }
  return -1;
}
...
async function canParse(code: string, theme: Theme) {
  ...
  const anyMermaid = mermaid as { parse?: (source: string) => Promise<unknown> };
...
self.onmessage = async (event: MessageEvent<RequestMessage>) => {
  ...
  try {
    if (message.action === 'canParse') {
      send({ id: message.id, ok: true, result: await canParse(message.payload.code, message.payload.theme) });
      return;
    }
    if (message.action === 'findPrefix') {
      send({ id: message.id, ok: true, result: await findLastRenderablePrefix(message.payload.code, message.payload.theme) });
      return;
    }
