 ...
 * MCP endpoint：`/office/mcp` → office 子 server，`/finance/mcp` → finance 子
 * server。每个路径是一个独立的 MCP endpoint / origin（客户端按 URL 连接，
 * 各自独立协商 —— 3.7 模式定义）。
 *
...
export async function createGateway(
    ...
    const bunServer = Bun.serve({
        ...
        async fetch(request) {
            ...
            if (!endpoint) {
                return new Response(
                    `MCPP gateway: no endpoint at '${pathname}'`,
                    { status: 404 },
                );
            }
