...
/** VS Code 风格文件/文件夹图标（按路径映射扩展名与特殊文件名）。 */
export const VSCodeFileIcon: Component<VSCodeFileIconProps> = (props) => {
  ...
  const icon = () => (props.directory
    ? FOLDER_ICONS[kind() as VSCodeFolderIconKind]
    : FILE_ICONS[kind() as VSCodeFileIconKind]);
  const size = () => props.size ?? 16;

  return (
    <span
      class={cn('inline-grid shrink-0 place-items-center', props.class)}
      data-file-icon={kind()}
      aria-hidden="true"
    >
