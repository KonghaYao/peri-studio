  ...
  return (
    <AccordionPrimitive.Header class="flex">
      <AccordionPrimitive.Trigger
        class={cn(
          'ui-accordion-trigger flex flex-1 cursor-pointer items-center justify-between border-0 border-b border-border-subtle bg-transparent py-12 text-left text-13 font-medium text-content-primary transition-colors outline-none duration-120 hover:text-accent-solid focus-visible:outline-2 focus-visible:outline-solid focus-visible:outline-focus-ring focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-45',
          local.class,
        )}
        {...rest}
      >
...
export function AccordionContent<T extends ValidComponent = 'div'>(props: PolymorphicProps<T, ContentProps<T>>) {
  ...
  return (
    <AccordionPrimitive.Content
      ...
      {...rest}
    >
      <div class="pb-12 pt-4">{local.children}</div>
    </AccordionPrimitive.Content>
  );
}
