...
export const WorkbenchRailButton: Component<WorkbenchRailButtonProps> = (props) => {
  ...
  return (
    <IconButton
      ...
      class={cn(
        'relative rounded-md border-0 bg-transparent',
        layout() === 'catalog'
          ? 'w-full min-h-36'
        ...
        local.active && indicatorSide() === 'left' && workbenchRailButtonActiveLeftClass,
        local.active && indicatorSide() === 'right' && workbenchRailButtonActiveRightClass,
        local.class,
      )}
    >
      {local.children}
      <Show when={(local.badge ?? 0) > 0}>
        <span class={workbenchRailButtonBadgeClass}>
