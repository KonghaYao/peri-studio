...
import { bindMcpAppHost } from './mcp-app-host';
import type { McpAppHostBindings, McpAppHostHandle, McpAppHostSessionView } from './types';
...
export interface McpAppFrameShellProps {
  session: () => McpAppHostSessionView | null;
  height: () => number;
  bindings: McpAppHostBindings;
  /** 测试钩子：覆盖默认 bindMcpAppHost。 */
  bindHost?: typeof bindMcpAppHost;
}

export function McpAppFrameShell(props: McpAppFrameShellProps) {
  ...
  const bindHost = () => props.bindHost ?? bindMcpAppHost;
  ...
  const syncIframeHost = () => {
    ...
    iframe.style.height = fullscreen() ? '100%' : `${props.height()}px`;
  };
  ...
  onMount(() => {
    ...
    const started = bindHost()(iframe, props.session, abort.signal, props.bindings).catch(() => null);
    onCleanup(() => {
      abort.abort();
      void started.then((handle: McpAppHostHandle | null) => handle?.close()).catch(() => undefined);
  ...
  const inlineHeight = () => props.height();

  return (
    <Dialog open={fullscreen()} onOpenChange={setFullscreen}>
      <div
        class="ui-mcp-app-frame w-full max-w-(--tool-activity-max)"
      ...
      >
        <div
          ...
          class="ui-mcp-app-inline relative w-full overflow-hidden rounded-9 border border-divider bg-surface"
        ...
        <Show when={fullscreen()}>
          <DialogContent class={FULLSCREEN_DIALOG_CLASS}>
            ...
            <div
              ...
              class="ui-mcp-app-fullscreen flex min-h-0 flex-1 flex-col bg-surface"
            />
          </DialogContent>
        </Show>
      </div>
