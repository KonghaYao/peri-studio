...
import { filePathBasename, vscodeFileIconKind, vscodeFolderIconKind } from './vscode-file-icons';
...
describe('VS Code file icon mapping', () => {
  ...
  it('preserves basename casing for display', () => {
    expect(filePathBasename('src/Health.ts')).toBe('Health.ts');
  });
});
