...
test('composer surface keeps slash overlays unclipped and menu-sized', () => {
  const extra = read(join(srcRoot, 'styles', 'extra.css'));
  assert.match(extra, /\.ui-composer-surface-v2\s*\{[^}]*overflow:\s*visible/s);
  assert.match(extra, /\.ui-composer-slash-overlay\s*\{[^}]*width:\s*var\(--container-slash-menu\)/s);
  assert.match(extra, /\.ui-composer-slash-popover\s*\{[^}]*width:\s*var\(--container-slash-menu\)/s);
  assert.doesNotMatch(extra, /\.ui-composer-slash-overlay\s*\{[^}]*inset-inline:\s*0/s);
  ...
  assert.match(extra, /\.ui-workbench-floating-panel\s*\{[^}]*padding-bottom:\s*var\(--workbench-panel-content-inset-bottom\)/s);
  assert.match(extra, /\.ui-workbench-floating-panel\s*\{[^}]*bottom:\s*var\(--workbench-panel-inset-block\)/s);
